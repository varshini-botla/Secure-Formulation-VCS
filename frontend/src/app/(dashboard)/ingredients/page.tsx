    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-foreground">Ingredient Registry</h1>
            <p className="text-muted-foreground mt-1">Chemical catalog and standardized unit management.</p>
          </div>
        </div>
        <Button className="bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-500/10" onClick={() => setIsModalOpen(true)}>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">Add a standardized chemical or solvent to the registry.</p>
              </div>
              <form onSubmit={handleCreate} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-muted-foreground">Unique Code</label>
                    <Input 
                      required
                      className="bg-background border-border text-foreground placeholder:text-muted-foreground focus-visible:ring-blue-500" 
                ...
                <div className="space-y-1">
                  ...
                  <textarea 
                    className="w-full bg-background border border-border rounded-md p-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-blue-500"
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-border">
                  ...
                  <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-500/10">Save Material</Button>
                </div>
              </form>
            </motion.div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          <motion.div
            ...
            whileHover={{ y: -5 }}
            className="bg-card border border-border rounded-2xl p-6 hover:border-blue-600/50 dark:hover:border-blue-400/50 transition-all cursor-pointer group relative overflow-hidden shadow-sm"
          >
             ...
             <div className="flex justify-between items-start mb-4 relative z-10">
                <div className="space-y-1">
                   <span className="text-[10px] font-bold text-blue-600 dark:text-blue-400 font-mono tracking-widest">{ing.code}</span>
                   <h3 className="font-bold text-lg text-foreground group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{ing.name}</h3>
                </div>
             ...
             <p className="text-xs text-muted-foreground line-clamp-2 mb-4 relative z-10">
                {ing.description || 'Standardized material used in various formulations.'}
             </p>

             <div className="flex items-center justify-between relative z-10">
                <span className="text-xs font-bold text-muted-foreground">Unit: {ing.unit}</span>
                <Button variant="ghost" size="sm" className="text-blue-600 dark:text-blue-400 group-hover:bg-blue-500/10">
